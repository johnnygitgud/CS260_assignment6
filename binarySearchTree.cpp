...
Node::Node(std::string name) : fileName(name), left(nullptr), right(nullptr) {}


// Implementations of FileBST methods
FileBST::FileBST() : root(nullptr) {
...
Node* FileBST::insert(Node* node, std::string fileName) {
    if (node == nullptr) {
...
Node* FileBST::findMin(Node* node) {
...
Node* FileBST::remove(Node* node, std::string fileName) {
    if (node == nullptr) {
        return node;
    }
    if (fileName < node->fileName) {
...
// void FileBST::inOrderTraversal(Node* node) {
//     if (node != nullptr) {
...
void FileBST::addFile(const std::string& fileName, const std::string& content) {
...
void FileBST::removeFile(const std::string& fileName) {
    std::filesystem::path filePath = desktopPath / fileName;
    // Delete the file
...
void FileBST::searchFile(const std::string& fileName) {
    Node* current = root;
